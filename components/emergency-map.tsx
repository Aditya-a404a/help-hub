...
const EmergencyMap = () => {
  ...
  const createSmoothPath = useCallback(
    (start: [number, number], end: [number, number], segments: number) => {
  ...
  const createSafeDetour = useCallback(
  ...
  const calculateSafeRoute = useCallback(
    async (start: [number, number], end: [number, number]) => {
      console.log("calculateSafeRoute called with:", {
        start,
        end,
        dangerZones,
      });
  ...
  const loadEmergencyData = useCallback(
    async (latitude: number, longitude: number) => {
      ...
      try {
        ...
        const [amenitiesResponse, reliefCentersResponse, floodAlertsResponse] =
          await Promise.all([
            disasterAPI
              .getNearbyAmenities(latitude, longitude, 10, undefined)
              .catch(() => null),
            disasterAPI
              .getNearbyReliefCenters(latitude, longitude, 10, 10)
              .catch(() => null),
        ...
        if (amenitiesResponse?.success && amenitiesResponse.data) {
          const amenityPoints: SafePoint[] = amenitiesResponse.data.map(
            (amenity) => ({
              id: `amenity-${amenity._id}`,
              name: amenity.name,
              coordinates: [amenity.latitude, amenity.longitude] as [
                number,
                number
              ],
              ...
              features: [
                ...
                amenity.resources?.doctors
                  ...
                  : "",
                amenity.resources?.ambulances
                  ? `${amenity.resources.ambulances} ambulances`
                  : "",
              ].filter(Boolean),
              type: "amenity",
        ...
        if (reliefCentersResponse?.success && reliefCentersResponse.data) {
          const reliefPoints: SafePoint[] = reliefCentersResponse.data.map(
            (center) => ({
              id: `relief-${center.id}`,
              name: center.name,
        ...
        if (floodAlertsResponse?.success && floodAlertsResponse.alerts) {
          const floodDangerZones: DangerZone[] = floodAlertsResponse.alerts.map(
            (alert) => ({
              id: `flood-${alert.id}`,
              name: `Flood Risk Area - ${alert.flood_probability * 100}%`,
              coordinates: [alert.latitude, alert.longitude] as [
                number,
                number
              ],
  ...
  useEffect(() => {
    ...
    document.addEventListener(
      "safePointClick",
      handlePopupButtonClick as EventListener
    );
    ...
    safePoints.forEach((point) => {
      ...
      marker.bindPopup(`
        <div style="min-width: 200px;">
          <h3 style="margin: 0 0 8px 0; color: #16a34a; font-weight: 600;">${
            point.name
          }</h3>
          <p style="margin: 0 0 4px 0; font-size: 14px;"><strong>Distance:</strong> ${
            point.distance
          } km</p>
          <p style="margin: 0 0 4px 0; font-size: 14px;"><strong>Capacity:</strong> ${
            point.capacity
          } people</p>
          ...
          <div style="margin-top: 12px; padding-top: 8px; border-top: 1px solid #e5e7eb;">
            <button 
              onclick="document.dispatchEvent(new CustomEvent('safePointClick', {detail: '${
                point.id
              }'}))"
              style="
                background: #16a34a; 
  ...
  useEffect(() => {
    ...
    mapInstanceRef.current.eachLayer((layer) => {
      if (
        ...
        (layer as { isRoute?: boolean }).isRoute
      ) {
          ...
          {isCalculatingRoute ? (
            ...
            <>
              <div className="text-xs text-blue-600 font-medium mb-1">
                Route Distance:{" "}
                {calculateRouteStats(routePath).distance.toFixed(2)} km
              </div>
              <div className="text-xs text-blue-600 font-medium mb-2">
                Estimated Time: {calculateRouteStats(routePath).time} min
                (walking)
              </div>
            </>
          ) : null}
