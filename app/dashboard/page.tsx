...
const ChennaiMapComponent = dynamic(
  () => import("@/components/ui/chennai-map"),
  {
    ssr: false,
    loading: () => (
...
const SocialMediaMonitor = dynamic(
  () => import("@/components/social-media-monitor"),
  {
    ssr: false,
    loading: () => (
      <div className="h-full bg-muted/20 rounded-lg border-2 border-dashed border-muted-foreground/30 flex items-center justify-center">
...
const RouteFinder = dynamic(() => import("@/components/route-finder"), {
  ssr: false,
  loading: () => (
    <div className="h-full bg-muted/20 rounded-lg border-2 border-dashed border-muted-foreground/30 flex items-center justify-center">
      <div className="text-center">
        <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-primary mx-auto mb-2"></div>
        <p className="text-xs text-muted-foreground">Loading Route Finder...</p>
      </div>
    </div>
  ),
});
...
export default function DDMADashboardPage() {
  ...
  const loadDDMAData = async () => {
    ...
    2    try {
      ...
      const alertsResponse = await disasterAPI.getFloodAlerts(
        "high",
        "Chennai",
        10
      );
      if (alertsResponse.success) {
        const transformedAlerts: DisasterAlert[] = alertsResponse.alerts.map(
          (alert) => ({
            id: alert.id,
            type: "flood",
            severity: alert.severity_level as
              | "low"
              | "medium"
              | "high"
              | "critical",
      ...
      const reliefResponse = await disasterAPI.getReliefCenters(
        "Chennai",
        true
      );
      if (reliefResponse.success) {
        const transformedTeams: ResponseTeam[] = reliefResponse.data.map(
          (center) => ({
            id: center.id,
            name: center.name,
            type: "rescue",
            location: center.address,
            coordinates: [center.latitude, center.longitude] as [
              number,
              number
            ],
      ...
      if (amenitiesResponse.success) {
        const transformedResources: ResourceInventory[] =
  ...
  const performSearch = (query: string) => {
    ...
    const filteredAlerts = disasterAlerts.filter(
      (alert) =>
    ...
    const filteredTeams = responseTeams.filter(
      (team) =>
    ...
    const filteredResources = resourceInventory.filter(
      (resource) =>
    ...
    const filteredCommunications = communicationChannels.filter(
      (channel) =>
    ...
    const matchingWidgets = Object.entries(widgetSearchMap).filter(
      ([, searchTerms]) =>
        searchTerms.some(
          (term) =>
            ...
            <div className="space-y-3">
                ...
                <Badge variant="outline" className="text-xs">
                  {(
                    disasterAlerts.reduce(
                      (sum, alert) => sum + alert.affectedPopulation,
                      0
                    ) / 1000
                  ).toFixed(1)}
                  K
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                <Badge variant="default" className="text-xs">
                  {
                    communicationChannels.filter((c) => c.status === "active")
                      .length
                  }
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <span className="font-bold">
                          🚨 CRITICAL ALERT: {criticalIncidents.length} Active
                          Emergency
                        </span>
                        <span className="ml-2 opacity-90">
                          {criticalIncidents[0].location} -{" "}
                          {criticalIncidents[0].affectedPopulation.toLocaleString()}{" "}
                          affected
                        </span>
                      </div>
                    </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <Badge variant="outline" className="text-xs">
                  ...
                  {selectedTab === "alerts" &&
                    (isSearching
                      ? searchResults.alerts.length
                      : disasterAlerts.filter((a) => a.status === "active")
                          .length) + " Active"}
                  {selectedTab === "teams" &&
                    (isSearching
                      ? searchResults.teams.length
                      : responseTeams.filter((t) => t.status === "deployed")
                          .length) + " Deployed"}
                  {selectedTab === "resources" &&
                    (isSearching
                      ? searchResults.resources.length
                      : resourceInventory.length) + " Items"}
          ...
          <div className="flex-1 overflow-hidden p-4">
            ...
            >
                          ...
                          <div className="grid grid-cols-2 gap-2 text-xs mb-3">
                            <div className="bg-red-50 dark:bg-red-950/20 p-2 rounded text-center">
                              <div className="text-red-600 font-bold text-lg">
                                {isSearching
                                  ? searchResults.alerts.filter(
                                      (a) => a.status === "active"
                                    ).length
                                  : disasterAlerts.filter(
                                      (a) => a.status === "active"
                                    ).length}
                              </div>
                            ...
                            <div className="bg-orange-50 dark:bg-orange-950/20 p-2 rounded text-center">
                              <div className="text-orange-600 font-bold text-lg">
                                {isSearching
                                  ? (
                                      searchResults.alerts.reduce(
                                        (sum, alert) =>
                                          sum + alert.affectedPopulation,
                                        0
                                      ) / 1000
                                    ).toFixed(1)
                                  : (
                    ...
                    {shouldShowWidget("Utility Vehicles") && (
                      ...
                      >
                        ...
                        <CardContent className="space-y-3">
                          ...
                          <div className="space-y-2">
                            {(isSearching ? searchResults.teams : responseTeams)
                              .slice(0, 3)
                              .map((team) => (
                                <div
                                  key={team.id}
                                ...
                                >
                                  ...
                                  <div className="flex-1">
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                      {team.status} • {team.personnel} personnel
                                    </p>
                                  </div>
                                  <Badge
                                    className={`text-xs ${
                      ...
                      >
                        ...
                        <CardContent className="space-y-3">
                          ...
                          <div className="text-xs text-muted-foreground text-center">
                            District Authority controls 6 vehicles across 5
                            departments
                          </div>
                        </CardContent>
                      </Card>
                          ...
                          <CardContent className="pt-6 pb-6">
                            <div className="text-center text-muted-foreground">
                              ...
                              <p className="text-lg font-medium">
                                No monitoring widgets match your search
                              </p>
                              <p className="text-sm">
                                Try searching for: social, ivr, response, or
                                weather
                              </p>
                            </div>
                          </CardContent>
              ...
              <TabsContent value="alerts" className="h-full m-0">
                {/* Alerts Tab */}
                <div className="h-full overflow-y-auto space-y-4 pr-2">
                  {(isSearching ? searchResults.alerts : disasterAlerts).map(
                    (alert) => (
                      <Card
                        key={alert.id}
                        ...
                        <CardHeader className="pb-3">
                          <div className="flex items-start justify-between">
                            ...
                            <div className="flex flex-col items-end gap-2 ml-4">
                              ...
                              <div
                                className={`w-3 h-3 rounded-full ${getResponseLevelColor(
                                  alert.responseLevel
                                )}`}
                              ></div>
                            </div>
                  ...
                  {(isSearching ? searchResults.alerts : disasterAlerts)
                    .length === 0 && (
              ...
              <TabsContent value="teams" className="h-full m-0">
                {/* Response Teams Tab */}
                <div className="h-full overflow-y-auto space-y-4 pr-2">
                  {(isSearching ? searchResults.teams : responseTeams).map(
                    (team) => (
                      <Card
                        key={team.id}
                        className="hover:shadow-md transition-shadow"
                        ...
                        <CardHeader className="pb-3">
                          <div className="flex items-start justify-between">
                            <div className="flex-1 min-w-0">
                              ...
                              <CardDescription className="text-xs mt-1">
                                {team.location} • {team.personnel} personnel •{" "}
              ...
              <TabsContent value="resources" className="h-full m-0">
                ...
                <div className="h-full overflow-y-auto space-y-4 pr-2">
                      ...
                      <CardHeader className="pb-3">
                        <div className="flex items-start justify-between">
                          <div className="flex-1 min-w-0">
                            ...
                            <CardDescription className="text-xs mt-1">
                              {resource.location} • {resource.quantity}{" "}
                              {resource.unit}
                            </CardDescription>
                          </div>
                          <div className="flex flex-col items-end gap-2 ml-4">
                  ...
                  {(isSearching ? searchResults.resources : resourceInventory)
                    .length === 0 && (
                    <div className="text-center text-muted-foreground py-8">
                      ...
                      <p className="text-sm text-muted-foreground">
                        {isSearching
                          ? "Try adjusting your search terms"
                          : "No resources currently in inventory"}
                      </p>
                    </div>
                  )}
                </div>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1 min-w-0">
                            <CardTitle className="text-sm flex items-center gap-2">
                              ...
                              {channel.type === "emergency_broadcast" && (
                  ...
                  {(isSearching
                    ...
                    <div className="text-center text-muted-foreground py-8">
                      ...
                      <p className="text-sm text-muted-foreground">
                        {isSearching
                          ? "Try adjusting your search terms"
                          : "No communication channels currently active"}
                      </p>
                    </div>
                  )}
                </div>
              ...
              <TabsContent value="route-finder" className="h-full m-0">
                {/* Route Finder Tab */}
                <div className="h-full overflow-y-auto space-y-4 pr-2">
                  <RouteFinder className="h-full" />
                </div>
              </TabsContent>
            </Tabs>
          </div>
